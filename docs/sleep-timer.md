...
指定時間に5秒のフェードアウトを開始し、再生停止後に画面を消灯・ロックする。既に再生が終わっている場合は音声を再開せず、消灯・ロックだけを行う。端末を再起動した場合は解除される。
...
アラームは非公開のBroadcastReceiverへ配送し、5秒のフェードに必要な間だけ最大10秒のpartial wake lockを取る。再生サービスが既に終わっているときに新しくサービスを起動したり無音再生したりしない。既存Playerへ接続しているときだけアプリ内音量を減衰させ、停止後またはキャンセル後に元の値を復元する。システム全体の音量や画面タイムアウト設定は変更しない。
