...
| 対象 | 確認した契約・処理 | 検証範囲 |
| --- | --- | --- |
| 新規認証 | `/oauth2/v2/auth`へ参照デスクトップの認可文脈と権限、PKCE・state・DPoP鍵識別子を渡す。同じ端末のループバックへ返ったコードを検証し、`/api/token`で交換する | razrの実アカウントでブラウザー認可、必要な転送権限の付与、保存後の転送HTTP 200を確認。以前の`/authorize`では同権限の要求が拒否された |
...
### 旧版から再生可能版への対応（2026-09-15）

旧版の曲を要求すると、`media`のキーが要求URIではなく再生可能版のURIになる場合がある。実機の保存アルバムで全16曲が旧実装の`Audio manifest is missing`になり、先頭曲の実応答で`item.metadata.uri`が応答のキーと一致し、`item.metadata.linked_from_uri`が要求URIを指すことを確認した。形式10の音声ファイルも返っていた。

`StreamingApiClient`は要求URIのエントリーを優先し、不在の場合だけ、キーと`metadata.uri`が一致する楽曲で`linked_from_uri`が要求URIを指す唯一の候補を選ぶ。無関係な先頭要素、対応の欠落、URI不一致、複数候補は採用しない。キャッシュは引き続き要求URIとアカウントに紐付け、音声ファイルのキャッシュは実際のファイルIDを使う。プレイリスト原本や表示中の曲IDを書き換えない。

保存プレイリストの別の旧版曲でも同じ構造と失敗を確認した。検索で見つけた同名曲は異なるURIで正常に再生できたため、作品名だけで再現対象を決めない。原本はGit管理外の`build/qa/jewels-manifest.json`と`fearless-manifest.json`、合成した回帰ケースは`StreamingApiClientTest`。認証・ライセンスの本文は記録していない。
...
連続した切替でライセンスがHTTP 429/503になった場合は、認証の再取得ではなく同じ要求・認証のまま待って送り直す。`Retry-After`（秒数またはHTTP日付、最大60秒）があれば従い、なければ10秒・30秒・60秒で最大3回送り直す。要求全体は4秒以上空け、同時要求の固まりで制限を延ばさない。再試行の所有権はこの取得処理に一本化し、プレイヤー側の即時再送は行わない。待ちきれない場合は切り替え先の取得を諦め、鳴っている曲を止めず切り替えの断念を通知する。
...
2026-09-25の追加記録では、Git管理外の`Downloads/flows_chrome`にある参照デスクトップのブラウザー認可を照合した。端末コード要求は行われていたが、成功した資格情報はコード＋PKCEの交換で発行されていた。`/oauth2/v2/auth`の要求は同じ公開client ID、28権限（再生転送を含む）、`creation_flow=desktop`、`creation_point`、`flow_ctx`、DPoP鍵識別子を伴う。捕捉要求の権限はカンマ区切り、`flow_ctx`は同じ値が2回入っていた。Playは標準の空白区切りと単一の`flow_ctx`で実機認可に成功した。Playの実機テストは既存セッションを書き換えずに新規認可と再生セッション生成を確認し、その後、通常の再認可画面から保存した資格情報でも転送HTTP 200を確認した。`flow_ctx`の時刻部分は捕捉した1例を基準にしているため、今後も異なる日時での再認可を検査する。
