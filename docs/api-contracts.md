...
連続した切替でライセンスがHTTP 429/503になった場合は、認証の再取得ではなく同じ要求・認証のまま待って送り直す。`Retry-After`（秒数またはHTTP日付、最大60秒）があれば従い、なければ10秒・30秒・60秒で最大3回送り直す。要求全体は4秒以上空け、同時要求の固まりで制限を延ばさない。待ちきれない場合は切り替え先の取得を諦め、鳴っている曲を止めず切り替えの断念を通知する。
