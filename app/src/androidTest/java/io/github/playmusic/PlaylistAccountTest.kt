...
class PlaylistAccountTest {
    ...
    fun privatePlaylistCreationMetadataArtworkTracksAndDeletion(): Unit = runBlocking {
        ...
        try {
            ...
            app.repository.setPlaylistMembership(target, selectedTrack, false)
            assertFalse(app.repository.playlistMembership(selectedTrack).single { it.playlist.uri == target.uri }.containsAll)
            val selectedAlbum = app.repository.search("jazz", io.github.playmusic.data.model.SearchFilter.ALBUMS).first()
            val albumUris = app.repository.detail(selectedAlbum).tracks.map { it.uri }.distinct()
            assertTrue(albumUris.isNotEmpty())
            app.repository.setPlaylistMembership(target, selectedAlbum, true)
            assertEquals(albumUris.toSet(), app.repository.detail(target, true).tracks.map { it.uri }.toSet())
            assertTrue(app.repository.playlistMembership(selectedAlbum).single { it.playlist.uri == target.uri }.containsAll)
            app.repository.setPlaylistMembership(target, selectedAlbum, false)
            assertTrue(app.repository.detail(target, true).tracks.isEmpty())
