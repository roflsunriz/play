...
- 2026-09-16、連続再生は成功するがシークや曲切替で同エラーになる場合があった。原因は連続取得によるライセンスのHTTP 429であり、認証の期限・更新とは無関係。429/503は認証再取得ではなく同じ要求・認証で送り直し（`Retry-After`優先、上限60秒、既定10秒・30秒・60秒で最大3回）、`LicensePacer`で要求全体を4秒以上空ける。重なり中の取得不能確定時は鳴っている曲を止めず切り替えを断念して通知する。回帰は`LicenseHttpClientTest`の再試行・間隔・原因連鎖検査。
